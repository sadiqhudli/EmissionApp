...
fun SignupScreen(
    ...
    val authSignRes = viewModel?.signUnFlow?.collectAsState()
    ...
    Column(
        ...
        EmailTextField(
            value = name, onValueChange = { name = it }, labelName = "name"
        )
        Spacer(modifier = Modifier.heightIn(20.dp))

        EmailTextField(
            value = email, onValueChange = { email = it }, labelName = "email"
        )
        Spacer(modifier = Modifier.heightIn(20.dp))

        EmailTextField(
            value = password, onValueChange = { password = it }, labelName = "password"
        )
        Spacer(modifier = Modifier.heightIn(20.dp))
        Button(
            onClick = {
    ...
    authSignRes?.value.let {
        when (it) {
            is Resource.Failure -> {
                Toast.makeText(
                    LocalContext.current, "${it.exception.message.toString()}", Toast.LENGTH_SHORT
                ).show()
...
@Composable
fun AppTheme1(content: @Composable () -> Unit) {
